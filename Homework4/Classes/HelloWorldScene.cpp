...
bool HelloWorld::init()
{
	...
	if (!database->getBoolForKey("isExist")) {
		...
		database->setIntegerForKey("killcount", 0);
	}
	...
	*/
	...
	//hittedNum
	HittedNum = Label::createWithTTF("0", "fonts/arial.ttf", 36);
	HittedNum->setPosition(Vec2(visibleSize.width / 2 + 100, visibleSize.height - 30));
	this->addChild(HittedNum);
...
void HelloWorld::updateCustom(float dt) {
	...
	if (pT->getPercentage() <= 0 && neverDead) {
		neverDead = false;
	...
	char num[5];
	_itoa(database->getIntegerForKey("killcount"), num, 10);
	HittedNum->setString(num);
	...
	if (pT->getPercentage() > 0) {
		if (Factory::getInstance()->getMonsterAmount() <= 8) {
			//create monster
			auto fac = Factory::getInstance();
			auto m = fac->createMonster();
			float x = random(origin.x, visibleSize.width);
...
void HelloWorld::xCallBack(Ref * pSender) {
	if (player->getSpriteFrame() == frame0 && pT->getPercentage() > 0) {
		...
		Rect attakRect = Rect(playerRect.getMinX() - 40, playerRect.getMinY(),
			playerRect.getMaxX() - playerRect.getMinX() + 80,
			playerRect.getMaxY() - playerRect.getMinY());
		auto fac = Factory::getInstance();
		Sprite* collision = fac->collider(attakRect);
		if (collision != NULL) {
			
			int num = database->getIntegerForKey("killcount") + 1;
			database->setIntegerForKey("killcount", num);
...
void HelloWorld::hitByMonster(float dt) {
	...
	if (collision != NULL) {
		pT->runAction(CCProgressTo::create(dt, pT->getPercentage() - 20));
		Factory::getInstance()->removeMonster(collision);
	}
}
